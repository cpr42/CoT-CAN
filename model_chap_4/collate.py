...
def pad_seqs(seqs, tensor_type):
      ...
      seq_lenths = torch.LongTensor(list(map(len, seqs))) # for s in seqs do len(s)
      max_seq_len = seq_lenths.max() # padding to max length
      ...
      for i, (seq, seq_len) in enumerate(zip(seqs, seq_lenths)):
        # tensor[i,a:b]：第i行、第a列到第b列（不包括第b列）的部分
            seq_tensor[i,:seq_len] = torch.tensor(seq, dtype = tensor_type)
            if (seq == [0,0,0]): # processing new dataset without contexts
                  mask[i,:seq_len] = torch.LongTensor([0]*int(seq_len))
            else:     
...
def collate(batch):
      ...
      if all('contexts_ids' in x for x in batch):
            input_ids_c, attention_mask_c = pad_seqs([x['contexts_ids'] for x in batch], tensor_type = torch.long)
      else:
           input_ids_c = None
           attention_mask_c = None
      if all('weights' in x for x in batch):
            weights = [x["weights"] for x in batch]
      else:
           weights = None
      item = {
      "events":events,
      ...
      "contexts_ids": input_ids_c,
      "contexts_masks": attention_mask_c,
      "weights": weights,
      }
      return item
