...
class Evaluator(object):
    ...
    def eval(self, y_true, y_pred, ind2label):
        ...
        true_labels = set(y_true) # ={0,1,2,3}
        for ind in ind2label:
            assert ind in true_labels # this evaluation only works when the dataset contains all the labels! would go wrong when not

        #individual precision and recall，of pos, neg and neu
        pres, recs, f1s, _ = precision_recall_fscore_support(y_true, y_pred, labels = list(sorted(ind2label.keys())), average = None);

        #total weighted precision recall，加权平均的指标
    ...
    def print_eval(self, eval, ind2label, best_eval = None):
        ...
        individual_eval, total_eval = eval # eval传入时是元组(individual_eval, total_eval)

        individual_pre, individual_rec, individual_f1 = individual_eval

        if best_eval is not None: # 在evaluate阶段固定为none
            best_individual_eval, best_total_eval = best_eval

        to_print = []

        score_dict = {"tot":{"f1":"0", "pre":"0", "rec":"0"}} # tot：储存加权平均指标
        for i in ind2label: # ind2label = {0:"pos", 1:"neg", 2: "neu"}
            score_dict[ind2label[i]] = {"f1":"0", "pre":"0", "rec":"0"}

        for i in range(len(individual_pre)): # len = class_num = 3
            class_name = ind2label[i]
            pre = individual_pre[i]
            pre_str = f"{pre*100:.2f}" # 略去百分号：90.89% -> 90.89
        ...
        # f"""....""" 形式的 f-strings 允许跨行编写字符串，不过这里并没有用到这个功能
        # ",".join([...])：以字符串形式拼接列表里的每个元素，并在每个元素之间插入一个逗号
