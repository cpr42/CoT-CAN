...
def predict(model, test_dataloader):
    ...
    with torch.no_grad():      
        ...
        pred_Y = [pred_Y[i] for i in range(0,len(pred_Y),args.contexts_num)]
        pred_probs = [pred_probs[i] for i in range(0,len(pred_probs),args.contexts_num)]
        # major_voting_end
...
def do_training(args):
    ...
    if args.dev_dir is not None:         
        ...
        input_ids = [e2id[e] for e in events for i in range(args.contexts_num)]
        data = {
            "events": [e for e in events for i in range(args.contexts_num)], 
            "labels": labels , # hard labels, each item is a label_str 
            "input_ids": input_ids,
    ...
    for cyc in tqdm.tqdm(range(args.iter), desc = "DEST CYCLE"): # desc参数：为进度条设置描述文字
        ...
        if cyc != args.iter - 1: # cyc:0~9
            ...
            ## pred_events = uevent_list #######
            
            # c2e_file = get_c2e(args.contexts_dir)
            # with open(c2e_file, "r") as f:
            #     contexts2events = json.load(f)
            # tmp = []
            # for e in pred_events:
            #     tmp.append(contexts2events[e])
            # pred_events = tmp

            for e, pred_prob, pred in zip(pred_events, pred_probs, pred_Y):
                label = ind2label[pred]
                # print(uevent2polar["I have mom"])
                # joint_prob = np.multiply(pred_prob, uevent2polar[e]) # uevent2polar:[event:[p_pos,p_neg,p_neu]]
                # joint_prob = joint_prob/np.sum(joint_prob)
                joint_prob = pred_prob
...
def get_args():
    ...
    parser.add_argument("--model_type", default = 'bert-base-uncased', choices = ["bert-small-uncased","bert-tiny-uncased","bert-base-uncased", "bert-base-cased", "bert-large-uncased", "bert-large-cased"])
